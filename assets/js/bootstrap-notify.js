...
(function (factory) {
	...
	var defaults = {
			...
			onClosed: null,
			icon_type: 'class',
			template: /*html*/`<div  data-notify="container" class="shadow-sm col-11 col-md-6 col-lg-5 col-xl-4 alert alert-{0} alert-dismissible fade show" role="alert">
	...
	$.extend(Notify.prototype, {
					...
					for (var command in commands) {
						switch (command) {
							case "type":
								...
								this.$ele.addClass('alert-' + commands[command]).find('[data-notify="progressbar"] > .progress-bar').addClass('bg-' + commands[command]);
								break;
							case "icon":
		...
		buildNotify: function () {
			...
			if ( !content.title) {
				this.$ele.find('[data-notify="title"]').remove();
			}
			if ( !content.icon) {
				this.$ele.find('[data-notify="icon"]').remove();
			}
		},
		setIcon: function() {
